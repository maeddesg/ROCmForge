...
|---:|---:|---:|---:|---:|---:|---:|
| 835 | 1422 | 587.2 | 5929 | 104440 | 56.8 | 106005 |

## Per-prompt metrics

| # | Name | Category | Prefill tok | Decode tok | Prefill tok/s | Decode tok/s | Total ms | EOS |
|---:|---|---|---:|---:|---:|---:|---:|:-:|
| 1 | Greeting | smoke | 24 | 15 | 261.4 | 61.3 | 346 | yes |
| 2 | Simple Sequence | smoke | 33 | 34 | 394.5 | 61.2 | 649 | yes |
| 3 | Prime Check (Python) | code_generation | 35 | 256 | 434.0 | 60.6 | 4317 | no |
| 4 | LRU Cache (C++) | code_generation | 51 | 512 | 646.2 | 58.1 | 8900 | no |
| 5 | REST API (Go) | code_generation | 66 | 1024 | 497.0 | 54.8 | 18843 | no |
| 6 | Mutex Explanation | prose | 33 | 56 | 431.7 | 61.8 | 993 | yes |
| 7 | TCP vs UDP | prose | 43 | 512 | 558.8 | 58.2 | 8884 | no |
| 8 | GPU Architecture Blog Post | prose | 62 | 1024 | 791.5 | 54.8 | 18757 | no |
| 9 | Binary Search Complexity | reasoning | 34 | 256 | 447.9 | 60.7 | 4301 | no |
| 10 | Debug Code | reasoning | 49 | 256 | 632.8 | 60.7 | 4302 | no |
| 11 | Distributed Message Queue | reasoning | 66 | 1024 | 523.7 | 54.8 | 18826 | no |
| 12 | Long System Prompt + Question | context_stress | 204 | 256 | 955.5 | 57.7 | 4661 | no |
| 13 | Long Output Story | context_stress | 64 | 512 | 808.3 | 57.7 | 8963 | no |
| 14 | Arithmetic (Q4_K Precision) | numerics | 33 | 64 | 439.4 | 62.7 | 1105 | no |
| 15 | Emoji/Special Characters | tokenizer_robustness | 38 | 128 | 505.1 | 61.7 | 2158 | no |
...
- **Prefill:** 24 tok in 92 ms (261.4 tok/s)
- **Decode:** 15 tok in 245 ms (61.3 tok/s) — hit EOS
...
- **Category:** smoke / **difficulty:** trivial / **max_tokens:** 64
- **Prefill:** 33 tok in 84 ms (394.5 tok/s)
- **Decode:** 34 tok in 556 ms (61.2 tok/s) — hit EOS
...
- **Prefill:** 35 tok in 81 ms (434.0 tok/s)
- **Decode:** 256 tok in 4227 ms (60.6 tok/s) — hit max_tokens
...
- **Prefill:** 51 tok in 79 ms (646.2 tok/s)
- **Decode:** 512 tok in 8811 ms (58.1 tok/s) — hit max_tokens
...
- **Prefill:** 66 tok in 133 ms (497.0 tok/s)
- **Decode:** 1024 tok in 18701 ms (54.8 tok/s) — hit max_tokens
...
- **Prefill:** 33 tok in 76 ms (431.7 tok/s)
- **Decode:** 56 tok in 907 ms (61.8 tok/s) — hit EOS
...
- **Prefill:** 43 tok in 77 ms (558.8 tok/s)
- **Decode:** 512 tok in 8798 ms (58.2 tok/s) — hit max_tokens
...
- **Prefill:** 62 tok in 78 ms (791.5 tok/s)
- **Decode:** 1024 tok in 18670 ms (54.8 tok/s) — hit max_tokens
...
- **Prefill:** 34 tok in 76 ms (447.9 tok/s)
- **Decode:** 256 tok in 4216 ms (60.7 tok/s) — hit max_tokens
...
- **Prefill:** 49 tok in 77 ms (632.8 tok/s)
- **Decode:** 256 tok in 4216 ms (60.7 tok/s) — hit max_tokens
...
- **Prefill:** 66 tok in 126 ms (523.7 tok/s)
- **Decode:** 1024 tok in 18690 ms (54.8 tok/s) — hit max_tokens
...
- **Prefill:** 204 tok in 214 ms (955.5 tok/s)
- **Decode:** 256 tok in 4437 ms (57.7 tok/s) — hit max_tokens
...
- **Prefill:** 64 tok in 79 ms (808.3 tok/s)
- **Decode:** 512 tok in 8874 ms (57.7 tok/s) — hit max_tokens
...
- **Prefill:** 33 tok in 75 ms (439.4 tok/s)
- **Decode:** 64 tok in 1020 ms (62.7 tok/s) — hit max_tokens
...
- **Prefill:** 38 tok in 75 ms (505.1 tok/s)
- **Decode:** 128 tok in 2074 ms (61.7 tok/s) — hit max_tokens
