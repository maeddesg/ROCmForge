...
|---:|---:|---:|---:|---:|---:|---:|
| 835 | 1429 | 584.3 | 5929 | 104875 | 56.5 | 106448 |

## Per-prompt metrics

| # | Name | Category | Prefill tok | Decode tok | Prefill tok/s | Decode tok/s | Total ms | EOS |
|---:|---|---|---:|---:|---:|---:|---:|:-:|
| 1 | Greeting | smoke | 24 | 15 | 262.3 | 61.4 | 345 | yes |
| 2 | Simple Sequence | smoke | 33 | 34 | 387.7 | 61.1 | 652 | yes |
| 3 | Prime Check (Python) | code_generation | 35 | 256 | 415.8 | 60.3 | 4336 | no |
| 4 | LRU Cache (C++) | code_generation | 51 | 512 | 636.7 | 57.9 | 8927 | no |
| 5 | REST API (Go) | code_generation | 66 | 1024 | 509.6 | 54.7 | 18860 | no |
| 6 | Mutex Explanation | prose | 33 | 56 | 436.9 | 61.5 | 996 | yes |
| 7 | TCP vs UDP | prose | 43 | 512 | 555.0 | 58.0 | 8919 | no |
| 8 | GPU Architecture Blog Post | prose | 62 | 1024 | 766.5 | 54.1 | 19004 | no |
| 9 | Binary Search Complexity | reasoning | 34 | 256 | 448.1 | 60.6 | 4312 | no |
| 10 | Debug Code | reasoning | 49 | 256 | 626.4 | 60.4 | 4324 | no |
| 11 | Distributed Message Queue | reasoning | 66 | 1024 | 519.4 | 54.7 | 18865 | no |
| 12 | Long System Prompt + Question | context_stress | 204 | 256 | 964.3 | 57.6 | 4667 | no |
| 13 | Long Output Story | context_stress | 64 | 512 | 803.0 | 57.7 | 8966 | no |
| 14 | Arithmetic (Q4_K Precision) | numerics | 33 | 64 | 434.9 | 62.4 | 1110 | no |
| 15 | Emoji/Special Characters | tokenizer_robustness | 38 | 128 | 496.6 | 61.6 | 2166 | no |
...
- **Prefill:** 24 tok in 92 ms (262.3 tok/s)
- **Decode:** 15 tok in 244 ms (61.4 tok/s) — hit EOS
...
- **Category:** smoke / **difficulty:** trivial / **max_tokens:** 64
- **Prefill:** 33 tok in 85 ms (387.7 tok/s)
- **Decode:** 34 tok in 557 ms (61.1 tok/s) — hit EOS
...
- **Prefill:** 35 tok in 84 ms (415.8 tok/s)
- **Decode:** 256 tok in 4243 ms (60.3 tok/s) — hit max_tokens
...
- **Prefill:** 51 tok in 80 ms (636.7 tok/s)
- **Decode:** 512 tok in 8837 ms (57.9 tok/s) — hit max_tokens
...
- **Prefill:** 66 tok in 130 ms (509.6 tok/s)
- **Decode:** 1024 tok in 18720 ms (54.7 tok/s) — hit max_tokens
...
- **Prefill:** 33 tok in 76 ms (436.9 tok/s)
- **Decode:** 56 tok in 911 ms (61.5 tok/s) — hit EOS
...
- **Prefill:** 43 tok in 77 ms (555.0 tok/s)
- **Decode:** 512 tok in 8832 ms (58.0 tok/s) — hit max_tokens
...
- **Prefill:** 62 tok in 81 ms (766.5 tok/s)
- **Decode:** 1024 tok in 18914 ms (54.1 tok/s) — hit max_tokens
...
- **Prefill:** 34 tok in 76 ms (448.1 tok/s)
- **Decode:** 256 tok in 4226 ms (60.6 tok/s) — hit max_tokens
...
- **Prefill:** 49 tok in 78 ms (626.4 tok/s)
- **Decode:** 256 tok in 4236 ms (60.4 tok/s) — hit max_tokens
...
- **Prefill:** 66 tok in 127 ms (519.4 tok/s)
- **Decode:** 1024 tok in 18729 ms (54.7 tok/s) — hit max_tokens
...
- **Prefill:** 204 tok in 212 ms (964.3 tok/s)
- **Decode:** 256 tok in 4446 ms (57.6 tok/s) — hit max_tokens
...
- **Prefill:** 64 tok in 80 ms (803.0 tok/s)
- **Decode:** 512 tok in 8877 ms (57.7 tok/s) — hit max_tokens
...
- **Decode:** 64 tok in 1025 ms (62.4 tok/s) — hit max_tokens
...
- **Prefill:** 38 tok in 77 ms (496.6 tok/s)
- **Decode:** 128 tok in 2080 ms (61.6 tok/s) — hit max_tokens
