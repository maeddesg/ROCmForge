...
|---:|---:|---:|---:|---:|---:|---:|
| 835 | 1413 | 590.9 | 5929 | 99255 | 59.7 | 100811 |

## Per-prompt metrics

| # | Name | Category | Prefill tok | Decode tok | Prefill tok/s | Decode tok/s | Total ms | EOS |
|---:|---|---|---:|---:|---:|---:|---:|:-:|
| 1 | Greeting | smoke | 24 | 15 | 262.7 | 64.9 | 332 | yes |
| 2 | Simple Sequence | smoke | 33 | 34 | 398.1 | 64.8 | 617 | yes |
| 3 | Prime Check (Python) | code_generation | 35 | 256 | 426.3 | 63.9 | 4099 | no |
| 4 | LRU Cache (C++) | code_generation | 51 | 512 | 643.7 | 61.1 | 8465 | no |
| 5 | REST API (Go) | code_generation | 66 | 1024 | 502.2 | 57.4 | 17984 | no |
| 6 | Mutex Explanation | prose | 33 | 56 | 423.1 | 64.3 | 958 | yes |
| 7 | TCP vs UDP | prose | 43 | 512 | 557.2 | 61.2 | 8452 | no |
| 8 | GPU Architecture Blog Post | prose | 62 | 1024 | 786.3 | 57.7 | 17837 | no |
| 9 | Binary Search Complexity | reasoning | 34 | 256 | 460.1 | 64.3 | 4067 | no |
| 10 | Debug Code | reasoning | 49 | 256 | 636.2 | 64.1 | 4082 | no |
| 11 | Distributed Message Queue | reasoning | 66 | 1024 | 577.7 | 57.6 | 17900 | no |
| 12 | Long System Prompt + Question | context_stress | 204 | 256 | 938.4 | 60.9 | 4432 | no |
| 13 | Long Output Story | context_stress | 64 | 512 | 799.4 | 61.0 | 8489 | no |
| 14 | Arithmetic (Q4_K Precision) | numerics | 33 | 64 | 442.4 | 66.3 | 1049 | no |
| 15 | Emoji/Special Characters | tokenizer_robustness | 38 | 128 | 508.1 | 65.2 | 2046 | no |
...
- **Prefill:** 24 tok in 91 ms (262.7 tok/s)
- **Decode:** 15 tok in 231 ms (64.9 tok/s) — hit EOS
...
- **Category:** smoke / **difficulty:** trivial / **max_tokens:** 64
- **Prefill:** 33 tok in 83 ms (398.1 tok/s)
- **Decode:** 34 tok in 525 ms (64.8 tok/s) — hit EOS
...
- **Prefill:** 35 tok in 82 ms (426.3 tok/s)
- **Decode:** 256 tok in 4007 ms (63.9 tok/s) — hit max_tokens
...
- **Prefill:** 51 tok in 79 ms (643.7 tok/s)
- **Decode:** 512 tok in 8377 ms (61.1 tok/s) — hit max_tokens
...
- **Prefill:** 66 tok in 131 ms (502.2 tok/s)
- **Decode:** 1024 tok in 17843 ms (57.4 tok/s) — hit max_tokens
...
- **Prefill:** 33 tok in 78 ms (423.1 tok/s)
- **Decode:** 56 tok in 871 ms (64.3 tok/s) — hit EOS
...
- **Prefill:** 43 tok in 77 ms (557.2 tok/s)
- **Decode:** 512 tok in 8365 ms (61.2 tok/s) — hit max_tokens
...
- **Prefill:** 62 tok in 79 ms (786.3 tok/s)
- **Decode:** 1024 tok in 17749 ms (57.7 tok/s) — hit max_tokens
...
- **Prefill:** 34 tok in 74 ms (460.1 tok/s)
- **Decode:** 256 tok in 3984 ms (64.3 tok/s) — hit max_tokens
...
- **Prefill:** 49 tok in 77 ms (636.2 tok/s)
- **Decode:** 256 tok in 3996 ms (64.1 tok/s) — hit max_tokens
...
- **Prefill:** 66 tok in 114 ms (577.7 tok/s)
- **Decode:** 1024 tok in 17777 ms (57.6 tok/s) — hit max_tokens
...
- **Prefill:** 204 tok in 217 ms (938.4 tok/s)
- **Decode:** 256 tok in 4205 ms (60.9 tok/s) — hit max_tokens
...
- **Prefill:** 64 tok in 80 ms (799.4 tok/s)
- **Decode:** 512 tok in 8399 ms (61.0 tok/s) — hit max_tokens
...
- **Prefill:** 33 tok in 75 ms (442.4 tok/s)
- **Decode:** 64 tok in 965 ms (66.3 tok/s) — hit max_tokens
...
- **Prefill:** 38 tok in 75 ms (508.1 tok/s)
- **Decode:** 128 tok in 1962 ms (65.2 tok/s) — hit max_tokens
