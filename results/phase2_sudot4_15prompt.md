...
|---:|---:|---:|---:|---:|---:|---:|
| 835 | 1428 | 584.6 | 5929 | 104465 | 56.8 | 106035 |

## Per-prompt metrics

| # | Name | Category | Prefill tok | Decode tok | Prefill tok/s | Decode tok/s | Total ms | EOS |
|---:|---|---|---:|---:|---:|---:|---:|:-:|
| 1 | Greeting | smoke | 24 | 15 | 261.5 | 61.6 | 345 | yes |
| 2 | Simple Sequence | smoke | 33 | 34 | 398.3 | 61.2 | 648 | yes |
| 3 | Prime Check (Python) | code_generation | 35 | 256 | 425.1 | 60.5 | 4320 | no |
| 4 | LRU Cache (C++) | code_generation | 51 | 512 | 647.4 | 58.1 | 8899 | no |
| 5 | REST API (Go) | code_generation | 66 | 1024 | 500.4 | 54.8 | 18821 | no |
| 6 | Mutex Explanation | prose | 33 | 56 | 423.6 | 61.6 | 996 | yes |
| 7 | TCP vs UDP | prose | 43 | 512 | 552.7 | 58.1 | 8897 | no |
| 8 | GPU Architecture Blog Post | prose | 62 | 1024 | 783.2 | 54.8 | 18775 | no |
| 9 | Binary Search Complexity | reasoning | 34 | 256 | 447.0 | 60.7 | 4302 | no |
| 10 | Debug Code | reasoning | 49 | 256 | 624.8 | 60.6 | 4315 | no |
| 11 | Distributed Message Queue | reasoning | 66 | 1024 | 525.9 | 54.8 | 18834 | no |
| 12 | Long System Prompt + Question | context_stress | 204 | 256 | 966.3 | 57.7 | 4660 | no |
| 13 | Long Output Story | context_stress | 64 | 512 | 800.8 | 57.8 | 8951 | no |
| 14 | Arithmetic (Q4_K Precision) | numerics | 33 | 64 | 430.6 | 62.5 | 1109 | no |
| 15 | Emoji/Special Characters | tokenizer_robustness | 38 | 128 | 485.2 | 61.7 | 2163 | no |
...
- **Decode:** 15 tok in 244 ms (61.6 tok/s) — hit EOS
...
- **Category:** smoke / **difficulty:** trivial / **max_tokens:** 64
- **Prefill:** 33 tok in 83 ms (398.3 tok/s)
...
- **Prefill:** 35 tok in 82 ms (425.1 tok/s)
...
- **Prefill:** 51 tok in 79 ms (647.4 tok/s)
...
- **Prefill:** 66 tok in 132 ms (500.4 tok/s)
- **Decode:** 1024 tok in 18680 ms (54.8 tok/s) — hit max_tokens
...
- **Prefill:** 33 tok in 78 ms (423.6 tok/s)
- **Decode:** 56 tok in 909 ms (61.6 tok/s) — hit EOS
...
- **Prefill:** 43 tok in 78 ms (552.7 tok/s)
- **Decode:** 512 tok in 8810 ms (58.1 tok/s) — hit max_tokens
...
- **Prefill:** 62 tok in 79 ms (783.2 tok/s)
- **Decode:** 1024 tok in 18686 ms (54.8 tok/s) — hit max_tokens
...
- **Decode:** 256 tok in 4217 ms (60.7 tok/s) — hit max_tokens
...
- **Prefill:** 49 tok in 78 ms (624.8 tok/s)
...
- **Prefill:** 66 tok in 125 ms (525.9 tok/s)
- **Decode:** 1024 tok in 18699 ms (54.8 tok/s) — hit max_tokens
...
- **Prefill:** 204 tok in 211 ms (966.3 tok/s)
- **Decode:** 256 tok in 4439 ms (57.7 tok/s) — hit max_tokens
...
- **Prefill:** 64 tok in 80 ms (800.8 tok/s)
- **Decode:** 512 tok in 8862 ms (57.8 tok/s) — hit max_tokens
...
- **Prefill:** 33 tok in 77 ms (430.6 tok/s)
- **Decode:** 64 tok in 1024 ms (62.5 tok/s) — hit max_tokens
...
- **Prefill:** 38 tok in 78 ms (485.2 tok/s)
- **Decode:** 128 tok in 2075 ms (61.7 tok/s) — hit max_tokens
